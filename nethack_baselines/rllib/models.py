...
from nethack_baselines.rllib.util.id_pairs import id_pairs_table
...
GLYPH_TYPE_STRATEGIES = {
    ...
    "all_cat": Targets(groups=1, subgroup_ids=3, colors=1, chars=2, specials=1, do_linear_layer=False),
}
...
class GlyphEmbedding(nn.Module):
    ...
    def __init__(self, glyph_type, dimension, device=None, use_index_select=None):
        ...
        embed = {}
        if strategy.glyphs:
            embed["glyphs"] = nn.Embedding(nethack.MAX_GLYPH, self._dim(strategy.glyphs))
        if strategy.colors:
...
class Crop(nn.Module):
    def __init__(self, height, width, height_target, width_target):
        ...
        height_grid = self._step_to_range(2 / (self.height - 1), height_target)[:, None].expand(
            -1, self.width_target
        )
